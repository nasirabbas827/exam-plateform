...
$username = $password = $email = $phone = $age = $selectedCategory = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["category"])) {
        $category_err = "Please select a test category.";
    } else {
    ...
    if (empty($username_err) && empty($password_err) && empty($email_err) && empty($phone_err) && empty($age_err) && empty($category_err)) {
        $sql = "INSERT INTO users (username, password, email, phone, age, selected_category) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ssssii", $param_username, $param_password, $param_email, $param_phone, $param_age, $param_category);
        $param_username = $username;
        $param_password = password_hash($password, PASSWORD_DEFAULT);
        $param_email = $email;
        $param_phone = $phone;
        $param_age = $age;
        $param_category = $selectedCategory;
        mysqli_stmt_execute($stmt);
...
?>
    ...
    <p class="mb-5 text-center">Already have an account? <a href="login.php">Login here</a></p>
</div>
